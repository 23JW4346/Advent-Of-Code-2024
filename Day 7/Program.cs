...
namespace Day_7
{
    internal class Program
    {
        ...
        {
            long answer = SOL1(GetPuzzleInput());
            Console.WriteLine("Part 1: " + answer);
            answer = SOL2(GetPuzzleInput());
        ...
        static bool GetOperation(long curVal, long target, List<long> remainingVals, bool part2 = false)
        {
            ...
            if (GetOperation(tempM, target, remainingVals.Skip(1).ToList(), part2)) return true;
            if (GetOperation(tempA, target, remainingVals.Skip(1).ToList(), part2)) return true;
            if (part2)
            {
                if (GetOperation(tempC, target, remainingVals.Skip(1).ToList(), true)) return true;
            }
